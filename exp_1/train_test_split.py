def train_test_split_for_time_series(dataset):
    ...
    train_set = (dataset[dataset["Week"] <= cutoff_date].copy())
    test_set = ( dataset[dataset["Week"] > cutoff_date].copy())

    print("\n Time-Based Split ")
    print("Cutoff:", cutoff_date)
    print("Train:",train_set["Week"].min(),"→",train_set["Week"].max())
    print("Test:",test_set["Week"].min(), "→",test_set["Week"].max())
